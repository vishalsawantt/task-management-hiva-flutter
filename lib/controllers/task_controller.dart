...
Future initLoad() async {
  ...
  //     tasks.value = List<Map<String, dynamic>>.from(res["content"]);

  //   } catch (e) {

  //     Utils.snackBarError("Error", "Failed to fetch tasks");
  ...
  if (online) {
    ...
    List cached = cachedRaw
        .map((e) => Map<String, dynamic>.from(e))
        .toList();

    List offlineOnly =
        cached.where((t) => t["isOffline"] == true).toList();
  ...
  //     Utils.snackBarError("Error", "Failed to create task");
  //   }
  // }
  Future createTask(String title, String description) async {
  ...
  //     Utils.snackBarError("Error", "Failed to update task");
  //   }
  // }
  Future updateTask({
  required int id,
...
List cached = offline.getCachedTasks()
    .map((e) => Map<String, dynamic>.from(e))
    .toList();
